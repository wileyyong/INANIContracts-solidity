...
/**
 * Gets deployed contract from address, that may or may not have constructor parameters.
 *
 * @param name the case sensitive name of the contract in the Solidity file.
 */
 export async function getContract<T extends DeployableContract<T>>(
    name: string,
    address: string
): Promise<T> {
    const factory = await ethers.getContractFactory(name)
    const contract = <T>(<unknown>await factory.attach(address))
